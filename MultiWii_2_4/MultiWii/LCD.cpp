...
const uint8_t PROGMEM myFont[][5] = {
...
{ // Sends a string of chars untill null terminator
  unsigned char i = 0;
  while (*string)
  {
    for (i = 0; i < 5; i++)
...
{ // num chars in graph; percent as 1 to 100
  if (!charsInitialized)
  {
    ...
    static byte less[8] = {
        0x00, 0x04, 0x0C, 0x1C, 0x0C, 0x04, 0x00, 0x15,
    };
    static byte grt[8] = {
        0x00, 0x04, 0x06, 0x07, 0x06, 0x04, 0x00, 0x15,
    };
...
void i2c_OLED_DIGOLE_send_string(const char *string)
{ // Sends a string of chars but not the null terminator
  i2c_rep_start(OLED_DIGOLE_ADDRESS << 1);
  i2c_write(0x00);
  while (*string)
  {
...
{ // prints a single char - should be printable
  i2c_rep_start(OLED_DIGOLE_ADDRESS << 1);
  i2c_write(0x00);
  i2c_write('T');
  i2c_write('T');
...
void LCDclear()
{
  ...
  LCDprint(0x1B);
  LCDprint(0x5B);
  LCDprintChar("1;1H"); //cursor top left
...
{ // Line = 1 or 2 - vt100 has lines 1-99
#ifdef DISPLAY_FONT_DSIZE
...
#elif defined(OLED_DIGOLE)
  i2c_OLED_DIGOLE_send_string("TP");
  i2c_write(0);
  i2c_write(line - 1); // first column is 0, line numbers start with 0
#endif
}
...
} // audio for errors only while armed
#elif defined(OLED_I2C_128x64)
void LCDattributesBold()
...
//LCDprint(0x1b); LCDprint('c'); //RIS
  ...
  LCDsetLine(4);
  LCDprintChar("To ENTER CONFIG      "); // 21 characters on each line
  LCDsetLine(5);
  LCDprintChar("YAW RIGHT & PITCH FWD");
  LCDsetLine(7);
  LCDprintChar("To SAVE CONFIG       ");
  LCDsetLine(8);
  LCDprintChar("YAW LEFT & PITCH FWD ");
#endif
...
void LCDbar(uint8_t n, uint8_t v)
{
  ...
  for (i = 0; i < j; i++)
    LCDprint(159); // full
  if (j < n)
    LCDprint(154 + (v * n * 5 / 100 - 5 * j)); // partial fill
  for (i = j + 1; i < n; i++)
    LCDprint(154); // empty
#elif defined(OLED_DIGOLE)
  uint8_t i, j = (n * v) / 100;
...
void output_mAh()
{
  ...
  if (conf.powerTrigger1)
  {
    ...
    {
      LCDattributesReverse();
    } // buzzer on? then add some blink for attention
#endif
...
{
  if (failsafeEvents || (i2c_errors_count >> 10))
  { // errors
...
// no break !!
  ...
  case '5':
  {
    ...
    i = index++ % 16;
    if (i == 0)
      linenr = 0; //vt100 starts linenumbering @1
    ...
    else
    {
      ...
      if (j < PRI_SERVO_FROM)
        index = 7 + PRI_SERVO_FROM;
      else if (j > PRI_SERVO_TO)
        index = 16;
...
// no break !!
  ...
  case '6':
  {
    ...
    if (i > 0)
      v = (analog.vbatcells[i] > analog.vbatcells[i - 1] ? analog.vbatcells[i] - analog.vbatcells[i - 1] : 0);
    line1[2] = digit10(v);
    line1[4] = digit1(v);
    ...
    //      #endif
    if (v > VBATNOMINAL / VBAT_CELLS_NUM)
