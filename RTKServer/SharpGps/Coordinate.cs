...
namespace SharpGis.SharpGps
{
    ...
    public class Coordinate
    {
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            {
                case "D":
                    sb.AppendFormat(formatProvider, "{0:0#.0000}бу{1} ", Math.Abs(Latitude), (Latitude > 0 ? "N" : "S"));
                    sb.AppendFormat(formatProvider, "{0:0##.0000}бу{1}", Math.Abs(Longitude), (Longitude > 0 ? "E" : "W"));
                    break;
                case "DM":
                    {
                        int latDeg, lonDeg;
                        ...
                        sb.AppendFormat(formatProvider, "{0:0#}бу{1:0#.000}'{2} ", latDeg, latMin, (north ? "N" : "S"));
                        sb.AppendFormat(formatProvider, "{0:0##}бу{1:0#.000}'{2}", lonDeg, lonMin, (east ? "E" : "W"));
                    }
                    break;
                case "DMS":
                    {
                        ...
                        sb.AppendFormat(formatProvider, "{0:0#}бу{1:0#}'{2:0#.0}\"{3} ", latDeg, latMin, latSec, (north ? "N" : "S"));
                        sb.AppendFormat(formatProvider, "{0:0##}бу{1:0#}'{2:0#.0}\"{3}", lonDeg, lonMin, lonSec, (east ? "E" : "W"));
                        break;
                    }
                default:
