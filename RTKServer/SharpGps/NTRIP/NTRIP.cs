...
namespace SharpGis.SharpGps.NTRIP
{
    ...
    public class NTRIPClient : IDisposable
    {
        ...
        byte[] rtcmDataBuffer = new byte[2048];

        public delegate void RawDataDelegate(object sender, byte[] data);
        public event RawDataDelegate NTripDataReceived;

        public string MostRecentGGA = SharpGis.SharpGps.NMEA.GPGGA.GenerateGPGGAcode(34.2, 108.9);
        /// <summary>
        ...
        /// </summary>
        /// <param name="mountPt">Resource to request. Leave blank to get NTRIP service data</param>
        private byte[] CreateRequest(string mountPt)
        {
           string msg= "GET /" + mountPt + " HTTP/1.0\r\n" + "User-Agent: NTRIP Client\r\n" + "Accept: */*\r\nConnection: close\r\n";
            ...
            if (!string.IsNullOrEmpty(MostRecentGGA))
                msg += MostRecentGGA + "\r\n";
        ...
        private void OnDataReceived(IAsyncResult ar)
        {
            ...
            if (iRx > 0) //if we received at least one byte
            {
                try
                {
                    byte[] temp = new byte[iRx];
                    Array.Copy(rtcmDataBuffer, temp, iRx);
                    if (NTripDataReceived != null)
                        NTripDataReceived(this, temp);
                    else
                    {
                        if (GPSHandler.GpsPort.IsPortOpen)
                            //Send RTCM data to GPS. We assume the data is valid RTCM
                            GPSHandler.GpsPort.Write(temp);
                    }
                }
