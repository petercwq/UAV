...
public void setup() {
  ...
  baudListbox.getCaptionLabel().set("BAUD_RATE").setColorBackground(red_);
  ...
  commListbox.getCaptionLabel().set("PORT COM").setColorBackground(red_);
...
"Select motor(s) and control throttle.").setPosition(xServ-200, yServ+10).moveTo("Motors");
  ...
  confINF[6].setLabel("MAG Decl").setMin(-20).setMax(20).setDecimalPrecision(1).setMultiplier(.1f).moveTo("Config");
...
/*****************************************************************/
    ...
    if (toggleSetSetting) {
      ...
      toggleRead=true;
      payload = new ArrayList<Character>();
      payload.add(PApplet.parseChar( round(confSelectSetting.getValue())) );
    ...
    if ((toggleWriteServo || toggleWriteServoLive )&& !toggleRead) {   // MSP_SET_SERVO_CONF
      ...
      if (multiType == AIRPLANE || multiType == PPM_TO_SERVO || multiType == HELI_120_CCPM || multiType == HELI_90_DEG){
        for( i=0;i<8;i++){
          int q= (int)ServoSliderMIN[i].getValue(); payload.add(PApplet.parseChar (q % 256) ); payload.add(PApplet.parseChar (q / 256)  ); // Min
          q= (int)ServoSliderMAX[i].getValue(); payload.add(PApplet.parseChar (q % 256) ); payload.add(PApplet.parseChar (q / 256)  ); // Max
          q= (int)(ServoSliderC[i].getValue()); payload.add(PApplet.parseChar (q % 256) ); payload.add(PApplet.parseChar (q / 256)  ); // Servo centers
  
          servoRATE[i] = PApplet.parseInt(RateSlider[i].getValue());
       ...
       //******************************************************************************************
        ...
        for( i=0;i<8;i++){ servoRATE[i]=round(RateSlider[i].getValue()); payload.add(PApplet.parseChar(servoRATE[i]));}// servoRATE
        for( i=0;i<8;i++) { int q= (int)(ServoSliderC[i].getValue())+512; payload.add(PApplet.parseChar (q % 256) ); payload.add(PApplet.parseChar (q / 256)  ); } // Servo centers...
 ...
 if(ServosActive){
   ...
   //******************************************************************************************
    ...
    if (toggleWrite) {
      ...
      // MSP_SET_RC_TUNING
      payload = new ArrayList<Character>();
      payload.add(PApplet.parseChar( round(confRC_RATE.getValue()*100)) );
      payload.add(PApplet.parseChar( round(confRC_EXPO.getValue()*100)) );
      payload.add(PApplet.parseChar( round(rollPitchRate.getValue()*100)) );
      payload.add(PApplet.parseChar( round(yawRate.getValue()*100)) );
      payload.add(PApplet.parseChar( round(dynamic_THR_PID.getValue()*100)) );
      payload.add(PApplet.parseChar( round(throttle_MID.getValue()*100)) );
      payload.add(PApplet.parseChar( round(throttle_EXPO.getValue()*100)) );
      ...
      byteP[4] = (round(confP[4].getValue()*100.0f));
      byteI[4] = (round(confI[4].getValue()*100.0f));

      byteP[5] = (round(confP[5].getValue()*10.0f));
      byteI[5] = (round(confI[5].getValue()*100.0f));
      byteD[5] = (round(confD[5].getValue()*10000.0f))/10;

      byteP[6] = (round(confP[6].getValue()*10.0f));
      byteI[6] = (round(confI[6].getValue()*100.0f));
      byteD[6] = (round(confD[6].getValue()*10000.0f))/10;
      ...
      for( i=0;i<4;i++) {int q= (int)(confINF[i].getValue()); payload.add(PApplet.parseChar (q % 256) ); payload.add(PApplet.parseChar (q / 256)  ); }
      
      // PermanentLog
      int nn= round(confINF[4].getValue()*10); payload.add(PApplet.parseChar (nn - ((nn>>8)<<8) )); payload.add(PApplet.parseChar (nn>>8));// f
      ...
      // MagDec
      nn= round(confINF[6].getValue()*10); payload.add(PApplet.parseChar (nn - ((nn>>8)<<8) )); payload.add(PApplet.parseChar (nn>>8)); // h

      
      // VBatscale
      nn= round(VBat[0].getValue()); payload.add(PApplet.parseChar (nn)); // i
      for( i=1;i<4;i++) { int q= PApplet.parseInt(VBat[i].getValue()*10); payload.add(PApplet.parseChar (q)); } // j,k,l
...
public void controlEvent(ControlEvent theEvent) {
  if (theEvent.isGroup()) 
  {
    if (theEvent.getName()=="portComList") 
      {
        InitSerial(theEvent.getGroup().getValue()); // initialize the serial port selected //<>//
      }
  }
   if (theEvent.isGroup()) 
   {
     if (theEvent.getName()=="baudList")
     {
       GUI_BaudRate=(int)(theEvent.getGroup().getValue());  // Set GUI_BaudRate to selected. //<>//
     }
   }
