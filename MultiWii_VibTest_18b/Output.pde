...
void initOutputMax() { // Change ChW: added function to init ESCs with max THROTTLE for setup funtions
  for(uint8_t i=0;i<NUMBER_MOTOR;i++)
    pinMode(PWM_PIN[i],OUTPUT);
  writeAllMotors(2000);
  delay(300);
...
void my_mixTable() { // new function for Vibtest instead of mixTable()
  uint8_t i;

  for (i = 0; i < NUMBER_MOTOR; i++) {
    if (i == (activeMotor - 1)) {
      motor[i] = constrain(rcData[THROTTLE], 1000, 2000);
    } else {
      motor[i] = 1000;
    }  
  }
...
void increaseActiveMotor() {  // new function for Vibtest 
  activeMotor++;
  if (activeMotor > NUMBER_MOTOR) activeMotor = 1;
  blinkLED(activeMotor, 600, 1); // indicate number of active motor 
}  

void decreaseActiveMotor() {  // new function for Vibtest 
  activeMotor--;
  if (activeMotor <= 0) activeMotor = NUMBER_MOTOR;
  blinkLED(activeMotor, 600, 1); // indicate number of active motor 
}  
