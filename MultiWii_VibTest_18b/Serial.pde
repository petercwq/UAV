...
void serialCom() {
  ...
  if ((!tx_busy) && Serial.available()) {
    ...
    case 'M': // Multiwii @ arduino to GUI all data
      ...
      for(i=0;i<3;i++) serialize16(accNoise[i]);
      for(i=0;i<3;i++) serialize16(gyroNoise[i]);
      ...
      serialize16(setupMode);              // debug2
      serialize16((initTime-launchTime)/1000);              // debug3
      serialize16(activeMotor);              // debug4
      serialize8('M');
